  ...
  <div id="app">
    <div class="resume">
      <div class="left">
        ...
        <ContextList title="About me" :icon="require('@/assets/title-contact.png')">
      ...
      <div class="right">
        ...
        <ContextList title="Education">
          <ListItemEducation slot="listItem" school="广东药科大学" major="经济学" date="2013.9 — 2017.6"></ListItemEducation>
        </ContextList>
        ...
        <ContextList title="Job Experience">
          <ListItemExperience 
    ...
    return {
      ...
      jobExperience: [
        { 
          ...
          content: `<pre style='line-height:27px;'>这个是一个SPA，开发人数共2人，技术栈有: express、Vue、Vuex、Vue-router、moment.js等。
我与项目组长分工合作，以模块分割任务，我主要负责的有财务管理、公司管理、系统设置等。
工作内容主要是与产品和设计那边沟通好页面设计，高效高质量的页面重构，以及一些动态效果；
与后台沟通接口问题，使用ajax调用后台接口，把数据渲染到页面；
编写通用的组件，让它具有一定的易使用性、易扩展性、易维护性；
使用nodejs来连接后台的redis，来做消息订阅，通知；
让用户可以选中多个订单来把多个订单的图片一起打包下载，主要使用了nodejs的async和request模块。
</pre>`
        },
        { 
          company: '公司内部使用的Bug搜索网站', job: 'https://log.wshuttle.com', href: 'https://log.wshuttle.com', isLink: true,  date: '2017.04—2017.05',
          content: `<pre style='line-height:27px;'>该项目是我个人单独开发的。用时2周。ElasticSearch作为一个搜索服务器。主要它能提供一个分布式多用户
能力的全文搜索引擎。技术栈主要使用了nodejs的ElasticSearch、express模块，页面渲染使用ejs模板引擎，
页面交互使用了jquery。因为之前没接触过这个，所以要看官方文档学习以及了解nodejs的ElasticSearch模块。
然后建立索引，操作增删改查等api来实现。
主要需要提供一个添加的接口以供公司内部收集bug使用，然后书写查询接口，多种标签下拉查询，数量查询等，和展示接口以
供页面ajax来调用。 统计页面也使用了highcharts来展示简单的统计图表。
</pre>`
        },
        { 
          company: '微信公众号',   date: '2017.08—2017.09',
          content: `<pre style='line-height:27px;'>开发人数共2人，用时2周。技术栈有: express、jquery、moment.js等。
我主要完成页面的任务，另外一个同事完成server端的部署（使用nodejs作为服务端）。
页面使用了ejs模板引擎。根据UI和产品文档完成页面设计，通过ajax完成接口数据展示。
使用了JS—SDK来完成录音、上传录音、预览照片、上传照片、获取地理位置等功能。
主要遇到的难点在：由于微信公众号的录音文件格式为amr格式，需要在其他平台听的话需要转为MP3格式。
后面了解到了fluent-ffmpeg能转变到mp3格式。最后通过fluent-ffmpeg来转为成MP3，转化过程中把chunk
拼接成完整的buffer，转为binary格式，通过oss上传到阿里云服务器。
</pre>`
        },
        { 
          company: '微信小程序',   date: '2017.12—2017.12',
          content: `<pre style='line-height:27px;'>开发人数共3人，时间为一周。因为微信小程序刚出就关注，所以被选作为负责人。
也是nodejs作为服务器。技术栈有: express、ws等。
由于另外2个同事要花时间阅读小程序文档，所以我分配了自己大部分的开发任务，以及帮助他们解决一些遇到
的问题。我主要完成了：根据UI设计，把相识可服用的成组件；完成了大部分的页面和功能（聊天，下单接单，
个人信息注册，列表和详情，收藏）；封装ws模块的监听、触发、发送等事件，主要用于实现简单的聊天功能；
